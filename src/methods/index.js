...
export function markdownParser(str) {
  ...
  const rules = [
    ...
    [
      /#{3}\s?([^\n]+)/g,
      '<h3 class="text-xl font-bold text-green-400">$1</h3>',
    ],

    [
      /#{2}\s?([^\n]+)/g,
      '<h2 class="text-2xl font-bold text-yellow-400">$1</h2>',
    ],
    ...
    [
      /(`)(\s?[^\n,]+\s?)(`)/g,
      '<a class="bg-gray-300 text-slate-900 rounded-md px-1">$2</a>',
    ],
    ...
    [
      /!\[([^\]]+)\]\(([^)]+)\s"([^")]+)"\)/g,
      '<img class="block w-full" src="$2" alt="$1" title="$3" />',
    ],
